...
#SBATCH -J pa1v2_tests_$n  # job name with core count
#SBATCH --output=./slurm_files/pa1v2_${n}_%j.out  # Output file name
...
EOF
    ...
    if [ "$n" -gt 16 ]; then
        echo "mpirun ./pa1v2" >> "job_${n}.sh"
    else
        echo "mpirun -np $n ./pa1v2" >> "job_${n}.sh"
    fi
